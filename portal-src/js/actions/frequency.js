...
(function () {
  ...
  function messageForError(code) {
    code = String(code || "");
    if (code === "frequency_not_allowed") return "That frequency isn’t available for this subscription.";
    if (code === "frequency_no_change") return "No changes needed.";
    if (code === "contract_not_found") return "We couldn’t find that subscription. Please refresh and try again.";
  ...
  function pickOptionalShop() {
    ...
    try {
      ...
      var v =
        (root && (root.getAttribute("data-shop") ||
  ...
  function normalizeNextInput(next) {
    ...
    var interval = normInterval(next && (next.interval != null ? next.interval : next.deliveryInterval));
    var intervalCount = toNum(next && (next.intervalCount != null ? next.intervalCount : next.deliveryIntervalCount), 0);
      ...
      return await busy.withBusy(ui, async function () {
        try {
          ...
          if (cur.interval && cur.intervalCount) {
            if (cur.interval === desired.interval && cur.intervalCount === desired.intervalCount) {
          ...
          if (!result.ok) {
            try { console.warn("[frequency] cache patch failed:", result.error); } catch (e2) {}
          }
          ...
          showToast(ui, "Sorry — we couldn’t update your billing frequency. Please try again.", "error");
          return { ok: false, error: String(e && e.message ? e.message : e) };
        }
      }, "Updating frequency…");
    } finally {
