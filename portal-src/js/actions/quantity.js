...
(function () {
  ...
  function isLinesConnectionShape(lines) {
    try { return !!(lines && typeof lines === "object" && Array.isArray(lines.nodes)); } catch (e) { return false; }
  }
  ...
  async function submitQuantityImpl(ui, payload) {
    ...
    if (!window.__SP.api || typeof window.__SP.api.postJson !== "function") throw new Error("API not available.");
      ...
      return await busy.withBusy(
        ...
        async function () {
          try {
            ...
            if (prevQty === newQty) {
              try { busy.showToast(ui, "Quantity updated.", "success"); } catch (eNoop) {}
            ...
            if (!result.ok) {
              try { console.warn("[quantity] cache patch failed:", result.error); } catch (e2) {}
            }

            try { syncInMemoryContract(contractGid, result.contract || null); } catch (eSync) {}
            refreshCurrentScreen(contractGid);

            try { busy.showToast(ui, "Quantity updated.", "success"); } catch (e3) {}

            return { ok: true, contract: result.contract || null, patch: patch };
          } catch (e) {
            try {
              busy.showToast(ui, "Sorry — we couldn’t update the quantity. Please try again.", "error");
            } catch (_) {}
            return { ok: false, error: String(e && e.message ? e.message : e) };
          }
        },
